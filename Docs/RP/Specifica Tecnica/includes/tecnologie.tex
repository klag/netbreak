...
\subsection{MeteorJS}
\begin{itemize}
	\item \textbf{Descrizione}: Meteor è un framework open source JajaScript scritto usando NodeJs\ped{G}. Esso permette lo sviluppo di applicazioni web cross-platform. Il framework integra MongoDB\ped{G} e utilizza il pattern publish-subcribe per la propagazione dei dati in tempo reale tra client e server. Per la parte client si è deciso di utilizzare il package Meteor-Angular, in modo da poter usare il framework AngularJS per lo sviluppo frontend.
