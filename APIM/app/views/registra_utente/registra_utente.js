...
angular.module('APIM.registra_utente', ['ngRoute'])

.config(['$routeProvider', function($routeProvider) {
  $routeProvider.when('/registra_utente', {
...
.controller('registra_utente_ctrl', function($scope, $http) {
  ...
  /*funzione che invia immagini/file a filehandler Jolie subito dopo che immagine caricata*/
   $scope.submitForm = function() {
        ...
        /*rimuovo errori residui dall'interfaccia grafica*/
        var errorslistnode = document.getElementById("errors_list");
        while (errorslistnode.firstChild) {
            errorslistnode.removeChild(errorslistnode.firstChild);
        }
        ...
        if ($scope.cognome == null ) {
          errors.push("Cognome obbligatorio"); 
          ok = false;
        }
        if ($scope.email == null  || !validateEmail($scope.email)) {
          errors.push("Email non fornita o non valida"); 
          ok = false;
        }
        if ($scope.cittadinanza == null ) {
          errors.push("Cittadinanza obbligatoria"); 
          ok = false;
        }
        if ($scope.aboutme == null ) {
          errors.push("About Me obbligatorio"); 
          ok = false;
        }
        if ($scope.linksito == null ) {
          errors.push("Link al Sito obbligatorio"); 
          ok = false;
        }
        if ($scope.password != $scope.repassword || !checkPassword($scope.password)) {
          errors.push("Password non coincidono o del formato sbagliato"); 
          ok = false;
        }
        if ($scope.paypal == null  || !validateEmail($scope.paypal)) {
          errors.push("Email Paypal non fornita o non valida"); 
          ok = false;
        }
        if ($scope.avataruri == null) {
          errors.push("immagine non e' stata caricata con successo"); 
          ok = false;
        }
        /*se ci sono errori fai vedere la lista di errori a interfaccia grafica*/
        if (!ok) {
          document.getElementById('errors').style.visibility = 'visible';
          var i = 0;
          for (i; i < errors.length; i++) { 
               var node = document.createElement("li");
               node.appendChild(document.createTextNode(errors[i]));   
    ...
    /*variabile dove salvo l'uri dell'immagina caricata sul mini-cloud Jolie*/
    ...
    $scope.uploadFile = function(element) {
        ...
        reader.onload = function(event) {
          ...
          $http({
              method  : 'POST',
              url     : 'http://localhost:8004/setFile', //location+operation Jolie
              processData: false,
              /*inserisco l'immagine secondo pratica http post piu' efficiente*/
              transformRequest: function (data) {
                  var formData = new FormData();
                  formData.append("file", $scope.image);  //'file' nome del sottotipo che Jolie si aspetta
                  return formData;  
              },  
              /*per i file inviati tramite form il Content-Type va messo undefined*/
              headers: { 'Content-Type': undefined }
           }).then(function(response){
              /*salvo uri del file ottenuto dalla response di Jolie*/
